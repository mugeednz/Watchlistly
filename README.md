# Watchlistly
**Watchlistly** is a modern movie discovery app built with **SwiftUI** and **MVVM architecture**.  
It allows users to explore movies, view detailed information, and create their personal watchlists using live data fetched from the web.

## Features
- **Browse Movies**: Discover movies from a vast online database.
- **Movie Details**: View poster, year, genre, IMDb rating, and synopsis.
- **Search Functionality**: Quickly find movies by title.
- **Modern UI**: Clean and responsive design using SwiftUI.
- **MVVM Architecture**: Well-structured, maintainable, and scalable code.
